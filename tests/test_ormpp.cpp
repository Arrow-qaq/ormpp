...
TEST_CASE("optional")
{
    ...
    if (postgres.connect(ip, username, password, db))
    {
        ...
        CHECK(vec1.front().empty_.has_value() == false);
        auto vec2 =
            postgres.query<test_optional>("select * from test_optional;");
        REQUIRE(vec2.size() > 0);
        CHECK(vec2.front().age.value() == 200);
        CHECK(vec2.front().name.value() == "purecpp");
        CHECK(vec2.front().empty_.has_value() == false);
...
TEST_CASE("query some")
{
    ...
    if (mysql.connect(ip, username, password, db))
    {
        ...
        CHECK(mysql.insert(v) == 3);
        auto vec1 =
            mysql.query<std::tuple<int>>("select count(1) from student");
        CHECK(vec1.size() == 1);
    ...
    if (postgres.connect(ip, username, password, db))
    {
        ...
        CHECK(postgres.insert(v) == 3);
        auto vec1 =
            postgres.query<std::tuple<int>>("select count(1) from student");
        CHECK(vec1.size() == 1);
        CHECK(std::get<0>(vec1[0]) == 3);
    ...
    if (sqlite.connect(db))
    {
        ...
        CHECK(sqlite.insert(v) == 3);
        auto vec1 =
            sqlite.query<std::tuple<int>>("select count(1) from student");
        CHECK(vec1.size() == 1);
        CHECK(std::get<0>(vec1[0]) == 3);
...
TEST_CASE("get insert id after insert")
{
    ...
    if (mysql.connect(ip, username, password, db))
    {
        ...
        CHECK(id == 3);
        id = mysql.get_insert_id_after_insert<person>(
            {{"purecpp"}, {"purecpp"}});
        CHECK(id == 5);
    }
#endif
    ...
    if (postgres.connect(ip, username, password, db))
    {
        ...
        CHECK(id == 3);
        id = postgres.get_insert_id_after_insert<person>(
            {{"purecpp"}, {"purecpp"}});
        CHECK(id == 5);
    }
#endif
    ...
    if (sqlite.connect(db))
    {
        ...
        CHECK(id == 3);
        id = sqlite.get_insert_id_after_insert<person>(
            {{"purecpp"}, {"purecpp"}});
        CHECK(id == 5);
    }
#endif
...
TEST_CASE("query tuple_optional_t")
{
    ...
    if (postgres.connect(ip, username, password, db))
    {
        ...
        auto vec = postgres.query<std::tuple<
    ...
    if (sqlite.connect(db))
    {
        ...
        auto vec = sqlite.query<std::tuple<
            tuple_optional_t, std::optional<std::string>, std::optional<int>>>(
            "select id,name,age,name,age from tuple_optional_t;");
        CHECK(vec.size() == 2);
        auto tp1 = vec.front();
